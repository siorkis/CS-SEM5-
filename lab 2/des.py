...
class DES:
    ...
    def key_gen(self):
        # Key generation
        # --hex to binary
        self.key = self.hex2bin(self.key)

        # getting 56 bit key from 64 bit using the parity bits
        self.key = self.permute(self.key, self.keyp, 56)

    def split(self):
        # Splitting
        left = self.key[0:28] # rkb for RoundKeys in binary
        right = self.key[28:56] # rk for RoundKeys in hexadecimal
    ...
    def encrypt(self):
        ...
        return self.bin2hex(cipher_text)
    ...
    def decryption(self, message):
        ...
        # Initial Permutation
        message = self.permute(message, self.initial_perm, 64)
        print("After initial permutation", self.bin2hex(message))
...
des = DES("123456ABCD132536", "AABB09182736CCDD")
...
print("Decryption")
text = des.decryption(cipher_text)
